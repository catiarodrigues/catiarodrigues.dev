...
const ThemeToggle = () => {
  ...
  return (
    <button
      ...
      className="ml-auto relative w-[56px] h-[28px] rounded-full bg-gray-200 dark:bg-gray-700 cursor-pointer transition-colors duration-300"
    >
      <span
        className={`absolute top-[4px] w-[20px] h-[20px] rounded-full bg-white shadow-sm flex items-center justify-center transition-all duration-300 ${
          theme === 'light' ? 'left-[4px]' : 'left-[32px]'
        }`}
      >
        {theme === 'light' ? (
